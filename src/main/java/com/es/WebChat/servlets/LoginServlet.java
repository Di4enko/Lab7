...
public class LoginServlet extends  ChatServlet{
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        pw.println("<form action='/WebChat/login.do' method='post'>Enter your name: <input type='text' name='name' value=''><input type='submit' value='Enter the chat'>");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (name==null || "".equals(name)) {
            errorMessage = "User name cant be empty!";
        } else {
            errorMessage = processLogonAttempt(name, request, response);
        }
        if (errorMessage!=null) {
            ...
            response.sendRedirect("/WebChat/login.do");
        } else {
    ...
    String processLogonAttempt(String name, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (aUser.getSessionId().equals(sessionId) || aUser.getLastInteractionTime() < (Calendar.getInstance().getTimeInMillis() - sessionTimeout*1000)) {
            ...
            return "Sorry, but username <strong>" + name + "</strong> already taken. Pleas, choose other username!";
        }
    }
}
